...
- 비지도학습 알고리즘은 출력에 대한 지식 없이 데이터에서 패턴을 추론한다.
  - 레이블 데이터를 요하지 않아서, 더 큰 데이터셋을 쉽게 사용할 수 있다.
## 8장. 비지도 학습: 군집화 (p.281)
...
![1](https://github.com/caesar-kim/Finance/blob/main/%EA%B0%80%EC%B9%98%ED%95%A8%EC%88%98%20%EC%8B%9D.png)
  - 할인계수 lambda는 0~1 사이를 갖고 미래 보상이 즉각적인 혜택을 제공하지 않으며 불확실성이 더 높을 수 있어서 미래 보상에는 불리하다.
  - 미래 보상은 가치함수에 대한 중요한 입력이다.
  - 가치함수는 미래보상 G_t의 예측을 통해 상태 매력도를 측정한다. s의 가치함수는 예상 수익률이고, 시간t에서 이 상태에 있으면 정책 pi를 사용한다.
