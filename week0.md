...
- ROC 곡선 아래 영역 area under ROC curve(AUC)
    - 이진 분류 문제에 적합한 평가 매트릭. ROC는 확률을 나타내는 곡선, AUC는 분리가능성의 정도 혹은 치수를 나타내는 곡선.
    - AUC는 모델이 클래스 얼마나 잘 구별하는지를 말해줌. AUC가 클수록 모델의 정확성이 높다는 것.
    - 0.5라면 아예 구별 능력이 없다는 것.
    - AUC를 확률적으로 해석하면 양성의 경우와 음성의 경우를 임의로 선택할 때, 분류기에 따라 양성의 경우가 음서으이 경우보다 우세할 확률.
- 혼동행렬
    - 학습 알고리즘의 성능을 펼쳐 보여줌.
    - TP FN
    - FP TN
    - 두 개 이상 분류를 갖는 모델 정확성 보여주기에 용이함. x축은 예측의 경우를, y축은 정확성 결과를 나타냄.
- 지도학습 분류 문제를 위한 평가 메트릭 선택
    - 분류 문제에 대한 평가 메트릭은 실제 업무 속성에 많이 좌우됨.
    - 재현율은 사기 감지 같은 거짓 음성이 높은 비용을 들일 수 있을 때 처럼.
### 모델 선택 (p.102)
...
2. 모델 균형
- 여러 요소를 절충해야 함.
    - ANN, SVM, 일부 앙상블 등으로 매우 정확한 예측 모델을 만들 수는 있지만, 단순성과 해석성이 부족하며 자원이 많이 들 수도 있다.
    - 예측 성능이 가장 중요한 목표라면 해석성이 낮은 모델 선택.
        - 설명할 필요가 없도록.
        - 하지만 해석성이 결정적인 경우도 있다.
    - 금융 분야에서는 해석성 중심 사례가 많다.
        - 개인 신용카드를 알고리즘으로 거절했다면, 그리고 법적 대응이 있다면 금융기관은 어떻게 결정 내렸는지 설명해야 한다.
        - 인공신경망으로는 설명이 거의 불가능하기에, 결정트리 기반 모델이 더 적합하다.
    - 모델의 특징에 따라 데이터에 내재된 다양한 패턴들에 적합하게 모델링을 한다.
        - 따라서 첫 단계는 어떤 모델을 가지고 어느 모델이 데이터셋에 내재된 구조를 효과적으로 포착하는지 찾아본다.
        - 모든 지도학습 기반 실전 문제에서는 이와 같은 접근법으로 ㅗ델을 선택한다.
